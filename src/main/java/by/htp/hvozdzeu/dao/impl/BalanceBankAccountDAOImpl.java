...
public class BalanceBankAccountDAOImpl extends BalanceAccountRowMapper implements BalanceBankAccountDAO, InstanceDAO {
    ...
    private static final String MSG_STATUS_RESPONSE_NOT_CORRECT_CV_CODE = "Transaction has been canceled duo to not correct cv-code.";
    ...
    public BalanceAccount balanceBankAccount(String tokenRest, String cardNumber, String appSecretCode) throws DAOException {
        ...
        if (authDAO.findToken(tokenRest) && appSecretCode.equals(APP_SECRET_CODE)) {
            ...
            try (PreparedStatement preparedStatement = connection.prepareStatement(SQL_GET_BALANCE_BY_CARD_NUMBER)) {
                preparedStatement.setString(1, decrypt(cardNumber, getSecretKey()));
        ...
        if (checkCVCode(cardNumber, cvCode) && authDAO.findToken(tokenRest) && appSecretCode.equals(APP_SECRET_CODE)) {
            ...
            if (currentBalance.intValue() > amount.intValue()) {
                ...
                try (PreparedStatement preparedStatement = connection.prepareStatement(SQL_UPDATE_BALANCE)) {
                    ...
                    preparedStatement.setString(2, decrypt(cardNumber, getSecretKey()));
                    preparedStatement.setString(3, decrypt(cvCode, getSecretKey()));
    ...
    public Response checkCreditCardBankAccount(String tokenRest, String cardNumber,
        ...
        if (authDAO.findToken(tokenRest) && appSecretCode.equals(APP_SECRET_CODE)) {
            LOGGER.info("Parameters is not empty");
            try {
    ...
    private boolean checkCVCode(String creditCardNumber, String cvCode) throws DAOException, SQLException {
        boolean result = false;
        Connection connection = dataBaseConnection.getConnection();
        try (PreparedStatement preparedStatement = connection.prepareStatement(SQL_CHECK_CV_CODE)) {
            preparedStatement.setString(1, decrypt(creditCardNumber, getSecretKey()));
            preparedStatement.setString(2, decrypt(cvCode, getSecretKey()));
            try (ResultSet resultSet = preparedStatement.executeQuery()) {
                if (resultSet.next()) {
